import { Customer, BookDetails, Borrow } from '../Models/model';
let url = "http://localhost:5260/api/libarary";
...
export async function fetchBooks(): Promise<[BookDetails]> {
    let apiURL = `${url}/bookscontroller/books`;
    let response = await fetch(apiURL);
    if (!response.ok) {
...
export async function getIndividualBook(bookID: number): Promise<BookDetails | null> {
    let apiURL = `${url}/bookscontroller/get/book/${bookID}`;
    let response = await fetch(apiURL);
    if (!response.ok) {
        return null;
    }
...
export async function addNewBook(book: BookDetails): Promise<string> {
    let apiURL = `${url}/bookscontroller/add/newBook`;
    let response = await fetch(apiURL, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
...
export async function editBookDetail(book: BookDetails): Promise<void> {
    let apiURL = `${url}/bookscontroller/new/book/edit`;
    let response = await fetch(apiURL, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
...
export async function deleteBookDetail(bookID: number): Promise<void> {
    ...
    const response = await fetch(`${url}/bookscontroller/delete/${bookID}`, {
        method: 'DELETE'
    });
    if (!response.ok) {
